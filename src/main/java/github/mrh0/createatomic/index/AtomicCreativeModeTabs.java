...
public class AtomicCreativeModeTabs {
    ...
    public static final RegistryObject<CreativeModeTab> MAIN_TAB = TAB_REGISTER.register("main",
            () -> CreativeModeTab.builder()
                    ...
                    .icon(AtomicBlocks.REACTOR_CASING::asStack)
    ...
    public static class RegistrateDisplayItemsGenerator implements CreativeModeTab.DisplayItemsGenerator {
        private List<Item> collectBlocks(RegistryObject<CreativeModeTab> tab, Predicate<Item> exclusionPredicate) {
            ...
            for (RegistryEntry<Block> entry : CreateAtomic.REGISTRATE.getAll(Registries.BLOCK)) {
                if (!CreateAtomic.REGISTRATE.isInCreativeTab(entry, tab)) continue;
        ...
        private List<Item> collectItems(RegistryObject<CreativeModeTab> tab, Predicate<Item> exclusionPredicate) {
            ...
            for (RegistryEntry<Item> entry : CreateAtomic.REGISTRATE.getAll(Registries.ITEM)) {
                if (!CreateAtomic.REGISTRATE.isInCreativeTab(entry, tab)) continue;
                Item item = entry.get();
        ...
        public void accept(CreativeModeTab.ItemDisplayParameters params, CreativeModeTab.Output output) {
            ...
            items.addAll(collectBlocks(MAIN_TAB, (item) -> {
                //if (item == AtomicBlocks.DIGITAL_ADAPTER.asItem()) return !CreateAddition.CC_ACTIVE;
                return false;
            }));
